function [dn] = incrementBenchmark(new_simulation_number,num_ABC_realisations,epsilon,budgets)

T = 200;
x0 = [1;0;0;0];
k_o = [1;0.025;100;0.25;1.9985;7.5e-5];
popsize = 10;
dn = '.';

%% Generate some new simulations if needed
if new_simulation_number>0
    produceBenchmarkData(new_simulation_number, popsize, T, x0, k_o);
end

%% Produce some ABC realisations from the benchmark
% Below, abc_post is the entire benchmark but randomly reordered
