...
export weight
function weight(w::AbstractWeight, m::M, y_obs)::NamedTuple{(:w,), Tuple{Float64}} where M
    return (w = w(y_obs; m...),)
end
function weight(w::AbstractWeight, mm::AbstractArray{M,1}, y_obs)::NamedTuple where M
    ww = Array{Float64,1}(undef, length(mm))
    save = weight!(ww, w, mm, y_obs)
    return merge((ww=ww,), save)
end
function weight!(ww::AbstractArray{Float64,1}, w::AbstractWeight, mm::AbstractArray{M,1}, y_obs)::NamedTuple where M
...
abstract type AbstractSimulator{T} end
abstract type AbstractComparison{T} end

struct LikelihoodFreeWeight{T, TF<:AbstractSimulator{T}, TC<:AbstractComparison{T}} <: AbstractWeight
    F::TF       # Simulator
    C::TC       # Comparison
...
function weight(w::LikelihoodFreeWeight{T}, m::M, y_obs)::NamedTuple where M where T
...
function simulate!(yy::AbstractArray{T,4}, F::AbstractSimulator{T}, m::M) where T where M
...
function simulate!(yy::AbstractArray{T,4}, F::AbstractSimulator{T}, mm::AbstractArray{M,1}) where T where M
...
function simulate!(y::AbstractArray{T,2}, F::AbstractSimulator{T}, m::M) where T where M
...
function simulate(F::AbstractSimulator{T}, m::M, K::Int64=1) where T where M
    dim = output_dimension(F)
    y = Array{T,2}(undef, dim, K)
    saved = simulate!(y, F, m)
    return merge((y=y,), saved)
end
function simulate(F::AbstractSimulator{T}, mm::AbstractArray{M,1}, K::Int64=1) where T where M
    dim = output_dimension(F)
