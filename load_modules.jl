...
K = PerturbationKernel{SingleCellModel_NoEF}(MvNormal(zeros(4), diagm(fill(0.1, 4))))
F = SingleCellSimulator(σ_init=0.1)
L_abc = ABCLikelihood(F, Euclidean(), 1.0, num_simulations=1*50) # integer multiple of 50
L_bsl = BayesianSyntheticLikelihood(F, num_simulations=50) # ideally 500, although can be slow

Σ_is = MonteCarloProposal(prior, L_bsl, NoEF_displacements)
Σ_mcmc = MonteCarloProposal(prior, PerturbationKernel{SingleCell}, L_bsl, NoEF_displacements)
